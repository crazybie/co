...
void GetUrl(string url, co::Action<ErrorCode, string> cb) {
  ...
  delay(3000, [=] {
    if (url.find("http") != 0)
      return cb(ErrorCode::InvalidParam, {});
...
class Handler {
 public:
  ...
  PromisePtr<bool> callInSequence() {
    ...
    string v;
    CoBegin(bool) {
      CoTryAwaitData(v, promised<string>(GetUrl, string("ttp://test2")),
                     (ExceptionWithCode & e) {
                       v = "";
...
PromisePtr<int> fs_open(const char* fname, int mode) {
  return uv_promised<int>(uv_fs_open, uv_fs_req_cleanup,
                          [](uv_fs_t* req, Action<int> ok, ErrorCb err) {
                            if (req->result < 0)
                              return err(make_exception_ptr(ExceptionWithCode(
                                  "fs_open error", (int)req->result)));
                            ok((int)req->result);
                          },
                          fname, mode, 0);
}
...
void testLibuv() {
  ...
  p->onDone([](string s) { printf("done: sz=%d\n", s.length()); });
